...
final_problem = File.readlines('input')
    .map do |line| Node.new(JSON.parse(line.strip), nil, 0).reduce end
    .reduce do |prev, problem|
      Node.new(problem, prev, 0).reduce
    end

pp "Final: #{final_problem.inspect}"
pp "M: #{final_problem.magnitude}"